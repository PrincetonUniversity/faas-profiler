...
def CompareArchives(archive_files, plot):
    ...
    for entry in archive_files:
        ...
        try:
            [test_name, config_df, stat_df, test_df, perf_mon_records] = pickle.load(
                open(pickle_file, "rb")
            )
        except:
...
def GetRawTestName(test_name):
    ...
    while True:
        ...
        if (end_pointer - start_pointer) > 0:
            raw_test_name = raw_test_name[:start_pointer] + raw_test_name[end_pointer:]
...
def RelativeDegradation(combined_stat_df):
    ...
    for test_cat in test_cats:
        ...
        sns.regplot(data=df, x="rate", y="throughput", ax=axs, order=2, truncate=True)
