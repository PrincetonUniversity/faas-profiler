...
def GetTestMetadata(
    test_metadata_file=FAAS_ROOT + "/synthetic_workload_invoker/test_metadata.out",
):
    """
...
def ConstructTestDataframe(since, limit=1000, read_results=False):
    ...
    if "error" in activations.keys():
        print(
            "Encountered an error getting data from the DB! Check the logs for more info."
        )
...
def CreateStatisticalSummary(test_df, config_df, test_start_time):
    ...
    for function in functions:
        ...
        try:
            rate = config_df[config_df["application"] == function]["rate"].tolist()
            start = config_df[config_df["application"] == function][
                "activity_window"
            ].tolist()
        ...
        try:
            throughput = (
                1000.0
                * len(partial_df["start"])
                / (partial_df["end"].max() - start_time)
            )
        except:
            throughput = 0
...
def GetControlGroupsRecords(since=None):
    ...
    scg_files = [
        ...
        if (isfile(join(log_path, f))) and ("systemd-cgtop" in f)
    ]

    # only keep the recent records
    scg_files = [
        f for f in scg_files if int(f[f.index("_") + 1 : f.index(".")]) >= since
    ]
...
def CapacityFactor(test_df):
    ...
    for function in functions:
        ...
        capacity_factors[function] = temp_data["latency"].quantile(0.9) - temp_data[
            "latency"
        ].quantile(0.1)
...
def main(argv):
    ...
    if FAAS_ROOT in config_file:
        ...
        [test_name, config_df] = ConstructConfigDataframe(FAAS_ROOT + "/" + config_file)

    read_results = True if options.read_results else False
    test_df = ConstructTestDataframe(
        since=test_start_time, limit=100000, read_results=read_results
    )
    if test_df is None:
    ...
    invocation_periods = [
        sorted_starts[i + 1] - sorted_starts[i] for i in range(len(sorted_starts) - 1)
    ]
    try:
        mean_invocation_period = 1.0 * sum(invocation_periods) / len(invocation_periods)
    ...
    if options.capacity_factor:
        with open(
            FAAS_ROOT + "/workload_analyzer/capacity_factors.json", "w"
        ) as outfile:
    ...
    print(
        "Warm Start Performance Summary (count: "
    ...
    print(
        "Cold Start Performance Summary (count: "
