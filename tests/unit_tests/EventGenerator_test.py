...
class TestEventGenerator(unittest.TestCase):
    def test_CreateEvents_ZeroDuration(self):
        inter_arrivals = CreateEvents(instance=0, dist='Uniform', rate=1, duration=0, seed=100)
        self.assertEqual(inter_arrivals, [])

    def test_CreateEvents_ZeroRate(self):
        inter_arrivals = CreateEvents(instance=0, dist='Uniform', rate=0, duration=5, seed=100)
        self.assertEqual(inter_arrivals, [])
    ...
    def test_CreateEvents_CheckLengthUniform(self):
        inter_arrivals = CreateEvents(instance=0, dist='Uniform', rate=1000, duration=1, seed=100)
        self.assertEqual(len(inter_arrivals), 1000)

    def test_CreateEvents_CheckPoissonCoverage(self):
        inter_arrivals = CreateEvents(instance=0, dist='Poisson', rate=100, duration=5, seed=100)
